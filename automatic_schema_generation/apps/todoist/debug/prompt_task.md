...
You are implementing the **tasks** resource for the Todoist API
    ...
    "Body_829067ab": {
      "properties": {
        ...
        "assignee_id": {
          ...
          "title": "Assignee Id",
          "description": "ID of the user to assign the task to. Pass null to clear the value. Omit this field to keep it unchanged.",
          "examples": [
            123456789
          ]
        },
...
todoist_tasks:
  user_id:
    ...
    target_column: id
    confidence: high
    reason: 'response schema: user_id'
  project_id:
    target_table: todoist_projects
    target_column: id
    confidence: high
    reason: 'response schema: project_id'
  section_id:
    target_table: todoist_sections
    target_column: id
    confidence: high
    reason: 'response schema: section_id'
  parent_id:
  ...
  added_by_uid:
    ...
    target_column: id
    confidence: high
    reason: 'response schema: added_by_uid'
  assigned_by_uid:
    target_table: todoist_user
    target_column: id
    confidence: high
    reason: 'response schema: assigned_by_uid'
  responsible_uid:
    target_table: todoist_user
    target_column: id
    confidence: high
    reason: 'response schema: responsible_uid'
  completed_by_uid:
    target_table: todoist_user
    target_column: id
    confidence: high
    reason: 'response schema: completed_by_uid'
  assignee_id:
    target_table: todoist_user
    target_column: id
    confidence: high
    reason: 'request body on POST /api/v1/tasks/{task_id}: assignee_id'
