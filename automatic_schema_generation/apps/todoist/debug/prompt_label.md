# Entity Implementation: labels

You are implementing the **labels** resource for the Todoist API
replica. You will add code to four existing files. Do not create new files.

## Context provided

### OpenAPI excerpt for labels
...
Resource `label` uses: alphabet=NUMERIC, length=10
...
Add a class `Label(Base)` with:

- Table name: `todoist_labels`
