# Entity Implementation (Pass 2 — Relationships): users

You are adding foreign key relationships to the **users** resource
(`AsanaUser` in `asana_users`). The base model, operations,
...
These resources have a demonstrated relationship with **users**
through shared endpoints, FK-shaped property names, or schema cross-references.

Direction key:
- **outgoing** — users's schemas contain fields that reference
  the related resource (e.g. a `_id` field or nested object pointing there).
  **Action**: add a FK column on `asana_users` pointing at the related
  resource's table, plus a `relationship()` on both sides.
- **incoming** — the related resource's endpoints reference users
  (the subject of those endpoints is the other resource, not users).
  **Action**: do NOT add a FK column on `asana_users`. The FK lives on the
  other resource's table. Add only a `relationship()` on the users
...
Evidence:
  - GET /teams/{team_gid}/users — url_segment: teams
  - GET /users/{user_gid}/teams — url_segment: users (incoming)
...
Evidence:
  - GET /users/{user_gid}/team_memberships — url_segment: users
  - GET /users/{user_gid}/user_task_list — url_segment: users
  - GET /users — url_segment: users
  - GET /users/{user_gid} — url_segment: users
  - PUT /users/{user_gid} — url_segment: users
  - GET /users/{user_gid}/favorites — url_segment: users
  - GET /teams/{team_gid}/users — url_segment: users
  - GET /workspaces/{workspace_gid}/users — url_segment: users
  - GET /workspaces/{workspace_gid}/users/{user_gid} — url_segment: users
  - PUT /workspaces/{workspace_gid}/users/{user_gid} — url_segment: users
  - GET /users/{user_gid}/workspace_memberships — url_segment: users

### workspaces
- Table: `asana_workspaces`
- Primary key: `gid`
- Direction: outgoing
- **Fields referencing workspaces**: `data.workspaces`
...
Evidence:
  - GET /users/{user_gid} — property: data.workspaces
  - PUT /users/{user_gid} — property: data.workspaces
  - GET /workspaces/{workspace_gid}/users — url_segment: workspaces
  - GET /workspaces/{workspace_gid}/users/{user_gid} — url_segment: workspaces
  - GET /workspaces/{workspace_gid}/users/{user_gid} — property: data.workspaces
  - PUT /workspaces/{workspace_gid}/users/{user_gid} — url_segment: workspaces
  - PUT /workspaces/{workspace_gid}/users/{user_gid} — property: data.workspaces
...
These schemas reference **users** but belong to entities that are
...
how users appears in the broader API.
