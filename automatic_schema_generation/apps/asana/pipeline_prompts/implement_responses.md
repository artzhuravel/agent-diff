Read the file `/Users/azh/agent-diff/automatic_schema_generation/apps/asana/pipeline_out/responses.json`. It contains the response definitions referenced by the resources being implemented from the Asana API spec, along with their referenced schemas.
...
Skip domain-specific or resource-specific responses — only implement responses that represent standard HTTP error patterns reusable across the endpoints being implemented.
