# Entity Implementation (Pass 2 — Relationships): teams

You are adding foreign key relationships to the **teams** resource
(`AsanaTeam` in `asana_teams`). The base model, operations,
...
These resources have a demonstrated relationship with **teams**
through shared endpoints, FK-shaped property names, or schema cross-references.

Direction key:
- **outgoing** — teams's schemas contain fields that reference
  the related resource (e.g. a `_id` field or nested object pointing there).
  **Action**: add a FK column on `asana_teams` pointing at the related
  resource's table, plus a `relationship()` on both sides.
- **incoming** — the related resource's endpoints reference teams
  (the subject of those endpoints is the other resource, not teams).
  **Action**: do NOT add a FK column on `asana_teams`. The FK lives on the
  other resource's table. Add only a `relationship()` on the teams
...
Evidence:
  - GET /teams/{team_gid}/projects — url_segment: teams (incoming)
  - POST /teams/{team_gid}/projects — url_segment: teams (incoming)
...
Evidence:
  - GET /teams/{team_gid}/custom_field_settings — url_segment: teams
  - GET /teams/{team_gid}/project_templates — url_segment: teams
  - GET /teams/{team_gid}/team_memberships — url_segment: teams
  - POST /teams — url_segment: teams
  - GET /teams/{team_gid} — url_segment: teams
  - PUT /teams/{team_gid} — url_segment: teams
  - GET /workspaces/{workspace_gid}/teams — url_segment: teams
  - GET /users/{user_gid}/teams — url_segment: teams
  - POST /teams/{team_gid}/addUser — url_segment: teams
  - POST /teams/{team_gid}/removeUser — url_segment: teams
...
Evidence:
  - GET /users/{user_gid}/teams — url_segment: users
  - GET /teams/{team_gid}/users — url_segment: teams (incoming)
...
Evidence:
  - GET /workspaces/{workspace_gid}/teams — url_segment: workspaces
...
These schemas reference **teams** but belong to entities that are
...
how teams appears in the broader API.
