# Entity Implementation (Pass 1 — Base): projects
...
- Model class: `AsanaProject`
- Primary key: `gid`

### Schemas

These component schemas represent **projects** in the API. Build your
...
bodies) but are not direct representations of **projects**. They
...
- Add a class `AsanaProject(Base)` with `__tablename__ = "asana_projects"`
...
- Use `generate_id("project")` for new IDs
...
- Include a `serialize_project_list()` for collection endpoints
...
projects in `database/schema.py`, marked with
`# STUB — expand when implementing this resource`. If you find a stub
for `AsanaProject`, **replace it** with the full implementation.
...
- Do not remove or modify existing *completed* implementations for other
  resources — but DO expand any stubs that exist for projects
