# Entity Implementation (Pass 1 — Base): teams
...
- Model class: `AsanaTeam`
- Primary key: `gid`

### Schemas

These component schemas represent **teams** in the API. Build your
...
Each entry below is an endpoint that operates on **teams**. Build
...
_Get a team's custom fields_
Parameters:
  - opt_fields (query, optional): array
Errors: 400, 401, 403, 404, 500

#### GET /teams/{team_gid}/project_templates
_Get a team's project templates_
Parameters:
...
_Get memberships from a team_
Parameters:
  - opt_fields (query, optional): array
Errors: 400, 401, 403, 404, 500

#### GET /users/{user_gid}/teams
_Get teams for a user_
Parameters:
  - opt_fields (query, optional): array
Errors: 400, 401, 403, 404, 500

#### GET /workspaces/{workspace_gid}/teams
_Get teams in a workspace_
Parameters:
  - opt_fields (query, optional): array
Errors: 400, 401, 403, 404, 500
...
bodies) but are not direct representations of **teams**. They
...
- Add a class `AsanaTeam(Base)` with `__tablename__ = "asana_teams"`
...
- Use `generate_id("team")` for new IDs
...
- Include a `serialize_team_list()` for collection endpoints
...
teams in `database/schema.py`, marked with
`# STUB — expand when implementing this resource`. If you find a stub
for `AsanaTeam`, **replace it** with the full implementation.
...
- Do not remove or modify existing *completed* implementations for other
  resources — but DO expand any stubs that exist for teams
