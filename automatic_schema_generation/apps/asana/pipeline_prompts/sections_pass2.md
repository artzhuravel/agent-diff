# Entity Implementation (Pass 2 — Relationships): sections

You are adding foreign key relationships to the **sections** resource
(`AsanaSection` in `asana_sections`). The base model, operations,
...
These resources have a demonstrated relationship with **sections**
through shared endpoints, FK-shaped property names, or schema cross-references.

Direction key:
- **outgoing** — sections's schemas contain fields that reference
  the related resource (e.g. a `_id` field or nested object pointing there).
  **Action**: add a FK column on `asana_sections` pointing at the related
  resource's table, plus a `relationship()` on both sides.
- **incoming** — the related resource's endpoints reference sections
  (the subject of those endpoints is the other resource, not sections).
  **Action**: do NOT add a FK column on `asana_sections`. The FK lives on the
  other resource's table. Add only a `relationship()` on the sections
...
Evidence:
  - GET /sections/{section_gid} — property: data.projects
  - PUT /sections/{section_gid} — property: data.projects
  - GET /projects/{project_gid}/sections — url_segment: projects
  - POST /projects/{project_gid}/sections — url_segment: projects
  - POST /projects/{project_gid}/sections — property: data.projects
  - POST /projects/{project_gid}/sections/insert — url_segment: projects
...
Evidence:
  - GET /sections/{section_gid} — url_segment: sections
  - PUT /sections/{section_gid} — url_segment: sections
  - DELETE /sections/{section_gid} — url_segment: sections
  - GET /projects/{project_gid}/sections — url_segment: sections
  - POST /projects/{project_gid}/sections — url_segment: sections
  - POST /sections/{section_gid}/addTask — url_segment: sections
  - POST /projects/{project_gid}/sections/insert — url_segment: sections
...
Evidence:
  - GET /sections/{section_gid}/tasks — url_segment: sections (incoming)
...
These schemas reference **sections** but belong to entities that are
...
how sections appears in the broader API.
