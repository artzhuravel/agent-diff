# Entity Implementation (Pass 2 — Relationships): workspaces

You are adding foreign key relationships to the **workspaces** resource
(`AsanaWorkspace` in `asana_workspaces`). The base model, operations,
...
These resources have a demonstrated relationship with **workspaces**
through shared endpoints, FK-shaped property names, or schema cross-references.

Direction key:
- **outgoing** — workspaces's schemas contain fields that reference
  the related resource (e.g. a `_id` field or nested object pointing there).
  **Action**: add a FK column on `asana_workspaces` pointing at the related
  resource's table, plus a `relationship()` on both sides.
- **incoming** — the related resource's endpoints reference workspaces
  (the subject of those endpoints is the other resource, not workspaces).
  **Action**: do NOT add a FK column on `asana_workspaces`. The FK lives on the
  other resource's table. Add only a `relationship()` on the workspaces
...
Evidence:
  - GET /workspaces/{workspace_gid}/projects — url_segment: workspaces (incoming)
  - POST /workspaces/{workspace_gid}/projects — url_segment: workspaces (incoming)
  - GET /workspaces/{workspace_gid}/projects/search — url_segment: workspaces (incoming)
...
Evidence:
  - GET /workspaces/{workspace_gid}/tags — url_segment: workspaces (incoming)
  - POST /workspaces/{workspace_gid}/tags — url_segment: workspaces (incoming)
...
Evidence:
  - GET /workspaces/{workspace_gid}/tasks/custom_id/{custom_id} — url_segment: workspaces (incoming)
  - GET /workspaces/{workspace_gid}/tasks/search — url_segment: workspaces (incoming)
...
Evidence:
  - GET /workspaces/{workspace_gid}/teams — url_segment: workspaces (incoming)

### users
- Table: `asana_users`
- Primary key: `gid`
- Direction: incoming
- **Fields referencing users**: `data.workspaces`
...
Evidence:
  - GET /users/{user_gid} — property: data.workspaces (incoming)
  - PUT /users/{user_gid} — property: data.workspaces (incoming)
  - GET /workspaces/{workspace_gid}/users — url_segment: workspaces (incoming)
  - GET /workspaces/{workspace_gid}/users/{user_gid} — url_segment: workspaces (incoming)
  - GET /workspaces/{workspace_gid}/users/{user_gid} — property: data.workspaces (incoming)
  - PUT /workspaces/{workspace_gid}/users/{user_gid} — url_segment: workspaces (incoming)
  - PUT /workspaces/{workspace_gid}/users/{user_gid} — property: data.workspaces (incoming)
...
Evidence:
  - GET /workspaces/{workspace_gid}/audit_log_events — url_segment: workspaces
  - GET /workspaces/{workspace_gid}/custom_fields — url_segment: workspaces
  - GET /workspaces/{workspace_gid}/typeahead — url_segment: workspaces
  - GET /workspaces/{workspace_gid}/workspace_memberships — url_segment: workspaces
  - GET /workspaces — url_segment: workspaces
  - GET /workspaces/{workspace_gid} — url_segment: workspaces
  - PUT /workspaces/{workspace_gid} — url_segment: workspaces
  - POST /workspaces/{workspace_gid}/addUser — url_segment: workspaces
  - POST /workspaces/{workspace_gid}/removeUser — url_segment: workspaces
  - GET /workspaces/{workspace_gid}/events — url_segment: workspaces
...
These schemas reference **workspaces** but belong to entities that are
...
how workspaces appears in the broader API.
